...
所有裝置的控制與狀態依序排在兩塊暫存器區，跟 PLC／EtherCAT 的過程映像一樣：

- **%Q 輸出區**＝控制（Python 或手動寫）；**%I 輸入區**＝狀態（機台回報）
- 位址：`%QX76.1`（位元）、`%QB76`（位元組）、`%QD20`（雙字；浮點變數當 REAL）

```python
twin.print_regs("Stopper01")            # 印出位址對照與目前值
twin.write_reg("%QX76.1", True)         # 擋料器伸出
twin.read_reg("%IX80.1")                # 伸出端到了沒
```

網頁右上角「**暫存器表**」開獨立頁面：列出全部暫存器、每個位元的意義（伸出、縮回端、偵測…），
輸出位元點一下就能手動切換（I/O 點檢），輸入以燈號即時顯示。產線分頁要開著。
手動切換和網頁 Python 可以同時用，互不覆蓋；本機 Python 模式下暫存器表只能看。

> **兩個視窗要並排、都看得見**：瀏覽器會暫停「切到背景的分頁」裡的 3D 模擬。
> 所以暫存器表是用獨立視窗開的（點產線頁右上角「暫存器表」），請把它和產線視窗並排擺放；
> 如果產線被蓋住或切到背景，暫存器表上方會提示。

## 看不懂裝置名稱？找出對應的零件

`MAIN.FG_Transport.Index03_Y_Stopper` 這種是 PLC 變數路徑，每個裝置都另外附有中文名稱與說明
（來自官方機台描述，例如「分度站 3 · 擋料氣缸」）：

- **暫存器表**：每個裝置都有中文名稱，可以用中文搜尋（例如「擋料氣缸」）。
  **滑鼠移到裝置上** → 3D 畫面把對應零件染成橘色；「**說明**」展開功能、訊號、注意事項；
  「**定位**」讓相機飛到那個零件。
- **3D 畫面**：左鍵點一個零件 → 零件變橘色，左上角出現說明卡，暫存器表同時捲到那個裝置。
- **Python**：

```python
dev = twin.device("MAIN.FG_Transport.Index03_Y_Stopper")
print(dev.label)                          # 分度站 3 · 擋料氣缸
print(twin.describe(dev.path))            # 功能、訊號、角色、注意事項
print_devices(twin, twin.devices("Index03"))   # 清單也會列出中文名稱
```
