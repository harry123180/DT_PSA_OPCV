...
namespace OC.UI.Console
{
	public class ConsoleMethodInfo
	{
		...
		public ConsoleMethodInfo( MethodInfo method, Type[] parameterTypes, object instance, string command, string signature, string[] parameters, string assembly = "")
		{
			this.method = method;
	...
	public static class DebugLogConsole
	{
		...
		// All the commands
		private static readonly List<ConsoleMethodInfo> _methods = new List<ConsoleMethodInfo>();
		private static readonly List<ConsoleMethodInfo> _matchingMethods = new List<ConsoleMethodInfo>( 4 );

		// All the parse functions
		private static readonly Dictionary<Type, ParseFunction> _parseFunctions = new Dictionary<Type, ParseFunction>()
		{
		...
		private static readonly Dictionary<Type, string> _typeReadableNames = new Dictionary<Type, string>()
		{
		...
		private static readonly List<string> _commandArguments = new List<string>( 8 );

		// Command parameter delimeter groups
		private static readonly string[] _inputDelimiters = new string[] { "\"\"", "''", "{}", "()", "[]" };

		// CompareInfo used for case-insensitive command name comparison
		internal static readonly CompareInfo _caseInsensitiveComparer = new CultureInfo( "en-US" ).CompareInfo;
		...
		static DebugLogConsole()
		{
...
#endif
			{
				...
				{
					if( _caseInsensitiveComparer.IsPrefix( assemblyName, ignoredAssemblies[i], CompareOptions.IgnoreCase ) )
					{
						ignoreAssembly = true;
						break;
					}
						...
						foreach( MethodInfo method in type.GetMethods( BindingFlags.Static | BindingFlags.Public | BindingFlags.DeclaredOnly ) )
						{
							foreach( object attribute in method.GetCustomAttributes( typeof( ConsoleMethodAttribute ), false ) )
							{
								...
								if( consoleMethod != null )
									AddCommand( consoleMethod.Command, consoleMethod.Description, method, null, consoleMethod.ParameterNames, method.DeclaringType.Namespace);
							}
						}
					}
				}
		...
		public static void LogAllCommands()
		{
			...
			{
				if( _methods[i].IsValid() )
					length += _methods[i].signature.Length + 7;
			}
		...
		public static void LogAllCommandsWithName( string commandName )
		{
			...
			FindCommands( commandName, false, _matchingMethods );
			if( _matchingMethods.Count == 0 )
				FindCommands( commandName, true, _matchingMethods );
			...
			else
			{
				...
				for( int i = 0; i < _matchingMethods.Count; i++ )
					commandsLength += _matchingMethods[i].signature.Length + 7;

				StringBuilder stringBuilder = new StringBuilder( commandsLength );
				stringBuilder.Append( "Matching commands:" );

				for( int i = 0; i < _matchingMethods.Count; i++ )
					stringBuilder.Append( "\n    - " ).Append( _matchingMethods[i].signature );
		...
		public static void AddCustomParameterType( Type type, ParseFunction parseFunction, string typeReadableName = null )
		{
			...
			_parseFunctions[type] = parseFunction;

			if( !string.IsNullOrEmpty( typeReadableName ) )
				_typeReadableNames[type] = typeReadableName;
		}
		...
		{
			_parseFunctions.Remove( type );
			_typeReadableNames.Remove( type );
		}
		...
		private static void AddCommand( string command, string description, MethodInfo method, object instance, string[] parameterNames, string assemblyName = "")
        {
			...
			for( int i = 0; i < parameters.Length; i++ )
			{
				...
				if( _parseFunctions.ContainsKey( parameterType ) || typeof( Component ).IsAssignableFrom( parameterType ) || parameterType.IsEnum || IsSupportedArrayType( parameterType ) )
			...
			else
			{
				...
				while( commandFirstIndex > 0 && _caseInsensitiveComparer.Compare( _methods[commandFirstIndex - 1].command, command, CompareOptions.IgnoreCase | CompareOptions.IgnoreNonSpace ) == 0 )
					commandFirstIndex--;
				while( commandLastIndex < _methods.Count - 1 && _caseInsensitiveComparer.Compare( _methods[commandLastIndex + 1].command, command, CompareOptions.IgnoreCase | CompareOptions.IgnoreNonSpace ) == 0 )
					commandLastIndex++;

				commandIndex = commandFirstIndex;
				for( int i = commandFirstIndex; i <= commandLastIndex; i++ )
				{
					int parameterCountDiff = _methods[i].parameterTypes.Length - parameterTypes.Length;
					if( parameterCountDiff <= 0 )
					{
						...
						{
							int j = 0;
							while( j < parameterTypes.Length && parameterTypes[j] == _methods[i].parameterTypes[j] )
								j++;
			...
			_methods.Insert( commandIndex, new ConsoleMethodInfo( method, parameterTypes, instance, command, methodSignature.ToString(), parameterSignatures, assemblyName ) );
		}
		...
		public static void RemoveCommand( string command )
		{
			if( !string.IsNullOrEmpty( command ) )
			{
				...
				{
					if( _caseInsensitiveComparer.Compare( _methods[i].command, command, CompareOptions.IgnoreCase | CompareOptions.IgnoreNonSpace ) == 0 )
						_methods.RemoveAt( i );
				}
			}
		}
		...
		public static string GetAutoCompleteCommand( string commandStart, string previousSuggestion )
		{
			...
			{
				commandIndex = ~commandIndex;
				return ( commandIndex < _methods.Count && _caseInsensitiveComparer.IsPrefix( _methods[commandIndex].command, commandStart, CompareOptions.IgnoreCase | CompareOptions.IgnoreNonSpace ) ) ? _methods[commandIndex].command : null;
			}

			// Find the next command that starts with commandStart and is different from previousSuggestion
			for( int i = commandIndex + 1; i < _methods.Count; i++ )
			{
				if( _caseInsensitiveComparer.Compare( _methods[i].command, previousSuggestion, CompareOptions.IgnoreCase | CompareOptions.IgnoreNonSpace ) == 0 )
					continue;
				else if( _caseInsensitiveComparer.IsPrefix( _methods[i].command, commandStart, CompareOptions.IgnoreCase | CompareOptions.IgnoreNonSpace ) )
					return _methods[i].command;
				else
					break;
			}
			...
			string result = null;
			for( int i = commandIndex - 1; i >= 0 && _caseInsensitiveComparer.IsPrefix( _methods[i].command, commandStart, CompareOptions.IgnoreCase | CompareOptions.IgnoreNonSpace ); i-- )
				result = _methods[i].command;
		...
		public static void ExecuteCommand( string command )
		{
			...
			FetchArgumentsFromCommand( command, _commandArguments );

			// Find all matching commands
			_matchingMethods.Clear();
			bool parameterCountMismatch = false;
			int commandIndex = FindCommandIndex( _commandArguments[0] );
			if( commandIndex >= 0 )
			{
				string _command = _commandArguments[0];

				int commandLastIndex = commandIndex;
				while( commandIndex > 0 && _caseInsensitiveComparer.Compare( _methods[commandIndex - 1].command, _command, CompareOptions.IgnoreCase | CompareOptions.IgnoreNonSpace ) == 0 )
					commandIndex--;
				while( commandLastIndex < _methods.Count - 1 && _caseInsensitiveComparer.Compare( _methods[commandLastIndex + 1].command, _command, CompareOptions.IgnoreCase | CompareOptions.IgnoreNonSpace ) == 0 )
					commandLastIndex++;

				while( commandIndex <= commandLastIndex )
				{
					if( !_methods[commandIndex].IsValid() )
					{
						_methods.RemoveAt( commandIndex );
						commandLastIndex--;
					}
					else
					{
						// Check if number of parameters match
						if( _methods[commandIndex].parameterTypes.Length == _commandArguments.Count - 1 )
							_matchingMethods.Add( _methods[commandIndex] );
						else
			...
			{
				string _command = _commandArguments[0];
				FindCommands( _command, !parameterCountMismatch, _matchingMethods );
				...
				else
				{
					...
					if( parameterCountMismatch )
						stringBuilder.Append( "ERROR: '" ).Append( _command ).Append( "' doesn't take " ).Append( _commandArguments.Count - 1 ).Append( " parameter(s). Available command(s):" );
					else
			...
			object[] parameters = new object[_commandArguments.Count - 1];
			string errorMessage = null;
			for( int i = 0; i < _matchingMethods.Count && methodToExecute == null; i++ )
			{
				ConsoleMethodInfo methodInfo = _matchingMethods[i];
				...
				for( int j = 0; j < methodInfo.parameterTypes.Length && success; j++ )
				{
					try
					{
						string argument = _commandArguments[j + 1];
        ...
        public static void FindCommands(string commandName, bool allowSubstringMatching, List<ConsoleMethodInfo> matchingCommands)
        {
            if (allowSubstringMatching)
            {
                for (int i = 0; i < _methods.Count; i++)
                {
                    ...
                    if (_methods[i].IsValid() && _methods[i].command.StartsWith(commandName))
                        matchingCommands.Add(_methods[i]);
                }
            }
            else
            {
                for (int i = 0; i < _methods.Count; i++)
                {
                    if (_methods[i].IsValid() && _caseInsensitiveComparer.Compare(_methods[i].command, commandName, CompareOptions.IgnoreCase | CompareOptions.IgnoreNonSpace) == 0)
                        matchingCommands.Add(_methods[i]);
                }
            }
        }
        ...
        internal static void GetCommandSuggestions( string command, List<ConsoleMethodInfo> matchingCommands, List<int> caretIndexIncrements, ref string commandName, out int numberOfParameters )
		{
			...
			for( int i = 0; i < command.Length; i++ )
			{
				...
				if( delimiterIndex >= 0 )
				{
					...
					if( !commandNameCalculated )
					{
						...
						if( commandName == null || commandNameLength == 0 || commandName.Length != commandNameLength || _caseInsensitiveComparer.IndexOf( command, commandName, i + 1, commandNameLength, CompareOptions.IgnoreCase | CompareOptions.IgnoreNonSpace ) != i + 1 )
				...
				else
				{
					...
					if( !commandNameCalculated )
					{
						...
						if( commandName == null || commandNameLength == 0 || commandName.Length != commandNameLength || _caseInsensitiveComparer.IndexOf( command, commandName, i, commandNameLength, CompareOptions.IgnoreCase | CompareOptions.IgnoreNonSpace ) != i )
			...
			if( !string.IsNullOrEmpty( commandName ) )
			{
				...
				if( !commandNameFullyTyped )
				{
					...
					if( commandIndex < _methods.Count && _caseInsensitiveComparer.IsPrefix( _methods[commandIndex].command, commandName, CompareOptions.IgnoreCase | CompareOptions.IgnoreNonSpace ) )
					{
						while( commandIndex > 0 && _caseInsensitiveComparer.IsPrefix( _methods[commandIndex - 1].command, commandName, CompareOptions.IgnoreCase | CompareOptions.IgnoreNonSpace ) )
							commandIndex--;
						while( commandLastIndex < _methods.Count - 1 && _caseInsensitiveComparer.IsPrefix( _methods[commandLastIndex + 1].command, commandName, CompareOptions.IgnoreCase | CompareOptions.IgnoreNonSpace ) )
							commandLastIndex++;
					}
					else
						commandLastIndex = -1;
				...
				else
				{
					...
					if( commandIndex < _methods.Count && _caseInsensitiveComparer.Compare( _methods[commandIndex].command, commandName, CompareOptions.IgnoreCase | CompareOptions.IgnoreNonSpace ) == 0 )
					{
						while( commandIndex > 0 && _caseInsensitiveComparer.Compare( _methods[commandIndex - 1].command, commandName, CompareOptions.IgnoreCase | CompareOptions.IgnoreNonSpace ) == 0 )
							commandIndex--;
						while( commandLastIndex < _methods.Count - 1 && _caseInsensitiveComparer.Compare( _methods[commandLastIndex + 1].command, commandName, CompareOptions.IgnoreCase | CompareOptions.IgnoreNonSpace ) == 0 )
							commandLastIndex++;
					}
					else
						commandLastIndex = -1;
				...
				{
					if( _methods[commandIndex].parameterTypes.Length >= numberOfParameters )
						matchingCommands.Add( _methods[commandIndex] );
				}
			}
		}
		...
		{
			for( int i = 0; i < _inputDelimiters.Length; i++ )
			{
				if( c == _inputDelimiters[i][0] )
					return i;
			}
		...
		{
			char startChar = _inputDelimiters[delimiterIndex][0];
			char endChar = _inputDelimiters[delimiterIndex][1];
		...
		{
			int min = 0;
			int max = _methods.Count - 1;
			while( min <= max )
			{
				int mid = ( min + max ) / 2;
				int comparison = _caseInsensitiveComparer.Compare( command, _methods[mid].command, CompareOptions.IgnoreCase | CompareOptions.IgnoreNonSpace );
				if( comparison == 0 )
					return mid;
		...
		public static bool IsSupportedArrayType( Type type )
		{
			...
			return _parseFunctions.ContainsKey( type ) || typeof( Component ).IsAssignableFrom( type ) || type.IsEnum;
		}

		public static string GetTypeReadableName( Type type )
		{
			string result;
			if( _typeReadableNames.TryGetValue( type, out result ) )
				return result;

			if( IsSupportedArrayType( type ) )
			{
				Type elementType = type.IsArray ? type.GetElementType() : type.GetGenericArguments()[0];
				if( _typeReadableNames.TryGetValue( elementType, out result ) )
					return result + "[]";
				else
		...
		public static bool ParseArgument( string input, Type argumentType, out object output )
		{
			...
			if( _parseFunctions.TryGetValue( argumentType, out parseFunction ) )
